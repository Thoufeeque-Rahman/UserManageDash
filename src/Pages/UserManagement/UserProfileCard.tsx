...
export default function UserProfileCard() {
    ...
    if (usersData.length === 0) {
        return (
            <div>
                ...
                {
                    data.map((user) => (
                        <Link to={`/user-management/user/${user.id}`} className='me-4' key={user.id} state={{ userData: user }}>
    ...
    return (
        <div className="max-h-max flex-1 rounded-xl bg-muted/50" >
            ...
            <Link to='/user-management/users' className='flex gap-2 p-4 border-t justify-center'>
